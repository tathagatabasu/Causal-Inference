...
\begin{tikzpicture}[scale=0.6]
    \begin{axis}[
        ...
        {exp(-0.5*(x^2 + y^2)/0.09) / (2*pi*0.09)};
...
\added{%
We may also use the empirically observed correlations from the data directly.
...
that is typically between $\underline{c}$ and $\overline{c}$.
Let $\overline{k}$ be the number of predictors with marginal correlation greater than $\underline{c}$
and $\underline{k}$ be number of predictors with marginal 
correlation greater than $\overline{c}$.
%%% MT to TB: in retrospect, using \underline{k} to lower bound the
%%% number of active variables is a bit strange... it seems like we're
%%% treating variables with "too high" correlations as inactive;
%%% should we simply set \underline{k} directly (e.g. 0, 1, or higher
%%% if suitable) instead of using the correlations?
As before, we can then consider $q\in \left[\underline{k}/p, \overline{k}/p\right]$.%
}
...
\begin{equation}
    \beta_{T}^D(q) = \left(\left(\hat{T}^D\right)^{\top} \hat{T}^D\right)^{-1}
    \left(\hat{T}^D\right)^{\top} \left[\hat{\beta}_{T}(q)\hat{T} 
...
Let $\overline{k}$ be the number of predictors with marginal correlation greater than $0.15$
and let $\underline{k}$ be number of predictors with marginal 

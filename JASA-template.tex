...
we have introduced a new simulation where we vary the number of predictors
...
\alert{of these 4 studies}, we generate the design matrix $X$ such that $X_i\sim\mathcal{N}(0, \Sigma)$
...
sub-cases: in \alert{case 1a} we consider all active variables to be confounders
and in the second \alert{case 1b} we consider some active variables which are only related to
the outcome model. 
\begin{description}
...
From the table it can be seen that for \added{case 1}, our method
...
for \added{cases 1a and 1b} respectively. However,
...
\begin{table}[ht]
\caption{Loss based on misspecification of active variables for \added{case 1a}.}\label{tab:misspec1}
\centering
\small
...
\begin{table}
\small
\caption{Loss based on misspecification of active variables for \added{case 1b}}
\label{tab:misspec2}
\begin{tabular}{|c||rrr|r||rr|r||rr|r||rr|r|}
  \hline
  &\multicolumn{4}{c||}{RBCE}&\multicolumn{3}{c||}{SSCE}
...
we omit the table of variable selection for \added{cases 2a and 2b}.

...
%%% MT to TB: changed "the variables and the outcome" to "a typically large number of predictor (or, explanatory) variables and the outcome of a treatment"
...
%%% MT to TB: using "predictors" again for consistency (and reduce confusion?)
predictors.
%confounders.
...
% on the $i$-th subject given that they received the treatment is the same as the (counterfactual) treatment effect when they remain as control \citep{winship99}.
...
\citep{xu2015} type model. This way, we can look into the posterior selection
...
\citep{ishwaran2005} prior for faster posterior
computation.

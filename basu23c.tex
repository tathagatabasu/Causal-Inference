...
be practical. Moreover, since there’s an aspect of intervention in causal effect 
estimator, we also need to be cautious about the side effects of a treatment.
Therefore, we are particularly interested in constructing a robust Bayesian framework
for variable selection. So that, once we are preparing a guideline for treatment, we 
can decide whether we require more data to conduct our analysis. For instance, 
we want to evaluate the problem based on the cost associated with different aspects
of variable selection including the possibility of abstention that is
conducting more tests. This way we can have a generalised utility based framework with
which we can interpret the lack of evidence efficiently without having an adverse
effect.
